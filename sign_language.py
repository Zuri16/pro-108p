    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            for id ,lm in enumerate(hand_landmark.landmark):
             ...
             # El código va aquí  
            x,y = int(lm_list[lm].x*w,int(lm_list[lm].y*h))
            cv2.circle(img,(x,y),15,(255,0,0),cv2.FILLED)

            if lm_list[lm].x < lm_list[lm-3].x:
            ...
            if all(finger_fold_status):
                if lm_list[thumb_tip].y < lm_list[thumb_tip-2].y:
                    print("Me gusta")
                    cv2.putText(img,"Me gusta",(20,30),cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),3)
                if lm_list[thumb_tip].y > lm_list[thumb_tip-1] > lm_list[thumb_tip-2].y:
                    print("No me gusta")
                    cv2.putText(img,"No me gusta",(20,30),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255),3)
